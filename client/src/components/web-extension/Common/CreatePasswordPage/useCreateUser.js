...
import { selectCreateWalletEncryptionType, selectCreateWalletKeyphrase, selectCreateWalletDerivationPath } from '@cd/selectors/createWallet';
...
const useCreateUser = () => {
	...
	console.log(`🚀 ~ useCreateUser ~ keyPhraseShares`, derivationPath);
	...
	const onCreateNewUser = useCallback(
		async (password) => {
			try {
				...
				const result = await createUserServiceSW(password, sharesToMnemonic(keyPhraseShares), encryptionType, derivationPath);
